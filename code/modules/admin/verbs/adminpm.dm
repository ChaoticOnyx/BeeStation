...
/client/proc/cmd_admin_pm(whom, msg)
	...
	if(!holder && !current_ticket)	//no ticket? https://www.youtube.com/watch?v=iHSPf6x1Fdo
		to_chat(src, "<span class='danger'>You can no longer reply to this ticket, please open another one by using the Adminhelp verb if need be.</span>", type = MESSAGE_TYPE_ADMINPM, confidential = TRUE)
		to_chat(src, "<span class='notice'>Message: [msg]</span>", type = MESSAGE_TYPE_ADMINPM, confidential = TRUE)
		return
	...
	if(external)
		...
		if(!msg)
			return
		if(holder)
			to_chat(src, "<span class='danger'>Error: Use the admin IRC/Discord channel, nerd.</span>", type = MESSAGE_TYPE_ADMINPM, confidential = TRUE)
			return
	...
	if(external)
		...
		if(recipient.holder)
			if(holder)	//both are admins
				...
				to_chat(recipient, "<span class='danger'>[replymsg]</span>", type = MESSAGE_TYPE_ADMINPM, confidential = TRUE)
