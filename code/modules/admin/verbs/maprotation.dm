...
/client/proc/forcemapconfig()
	...
	if(!check_rights(R_DEBUG) || !check_rights(R_SERVER) || !check_rights(R_PERMISSIONS))
		to_chat(src, "<span class='warning'>Insufficient rights (Requires debug, server and permissions).</span>", confidential = TRUE)
		return
	...
	if(!config.LoadConfig("_maps/[json_settings].json", TRUE))
		qdel(config)
		to_chat(usr, "<span class='warning'>Map json failed to load!</span>", confidential = TRUE)
		return
	SSticker.maprotatechecked = 1
